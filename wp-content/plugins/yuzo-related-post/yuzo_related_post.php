	...
	if( $wp_query->post_count != 0 ){ // if have result in loop post
		...
		if ( ( is_object($the_query_yuzo) && $the_query_yuzo->have_posts() && $wp_query->post_count != 0) || $counter_manual_post >= $metabox_add_post_first ) {
			...
			if( isset($yuzo_options->top_text) && $yuzo_options->top_text ){
				$_html .= "<div class='yuzo_clearfixed yuzo__title container ClanOT-Book'>". $this->IF_setHtml( $yuzo_options->top_text ) ."</div>";
			}
			...
			$_html                  .="<div class='yuzo_wraps container owl-carousel owl-theme post-carousel'>";
				...
				if( $yuzo_options->style == 1 ){
					...
					if( $yuzo_options->type_image == 'full-vertical' ){
						...
						$image = $this->IF_get_image(  $yuzo_options->thumbnail_size, $yuzo_options->default_image, get_the_ID(), $order_image );
						  $_html .= '
						  <div class="row">
						  <div class="relatedthumb relatedpost-'.get_the_ID().' '.$class_box_shadow.'">  
							  <div class="column-border">
							  <a '.$rel_link.' href="'.get_permalink().'" '.$target_link.' >
									  <div class="yuzo-img-wrap '.$css_shine_effect1.'" >
										'.$css_shine_effect2.'
										<div class="yuzo-img" style="background:url(\''.$image['src'].'\') 50% 50% no-repeat;height:300px;margin-bottom: 25px;background-size: '.$size_css.'; '.$css_border.'"></div>
									  </div>
									  '.$my_array_views['top'].'
								   <span class="multi-dim ClanOT-Medium">'.$if_utils->IF_setHtml( $if_utils->IF_cut_text( get_the_title(), $yuzo_options->text_length , true ) ).'</span>
							  '.$text2_extract .'
							  '.$my_array_views['bottom'].'
							  </a>
							  <div class="create ClanOT-Book">'
							  .  custom_echo( $post->post_content , 200) . '<a href="?p='. $post->ID .'"class="ClanOT-News"> <b>Read more</b></a>'.
							  '</div> 
							  <div class="flex date-cmnt">' 
							  . '<div class="left-side ClanOT-News">' .date('F j, Y',strtotime($post->post_date)).
							   '</div>'
							  . '<div class="right-side ClanOT-News">' . $post->comment_count .' Comments'.'</div></div>'.
		...
		if( $yuzo_options->display_random  ){
			...
			$count = 1;
			if( isset($yuzo_options->top_text) && $yuzo_options->top_text ){
				$_html .= "<div class='yuzo_clearfixed yuzo__title yuzo__title ClanOT-Book'>". $this->IF_setHtml( $yuzo_options->top_text ) ."</div>";
			}
